...
bool intersect(vec3 origin, vec3 direction, vec3 v0, vec3 v1, vec3 v2, out vec3 point)
{
 ...
 void main()
 {
	vec3 accelleration = vec3(0.0, -0.3, 0.0);// 加速度
	vec3 new_velocity = velocity + accelleration * time_step; // 新的速度
	vec4 new_position = position + vec4(new_velocity * time_step, 0.0); // 新的位置
	vec3 v0, v1, v2; // 顶点
	vec3 point;  // 新的位置
	int i;
	for(i = 0; i < triangle_count; i++) //获取三角形顶点
	{
