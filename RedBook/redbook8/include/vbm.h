...
typedef struct VBM_HEADER_t
{
	VBM_HEADER_t()
	{
		memset(this,0,sizeof(VBM_HEADER_t));
	}
    unsigned int magic;
    unsigned int size;
    char name[64];
...
typedef struct VBM_ATTRIB_HEADER_t
{
	VBM_ATTRIB_HEADER_t()
	{
		memset(this,0,sizeof(VBM_ATTRIB_HEADER_t));
	}
    char name[64];
    unsigned int type;
...
typedef struct VBM_FRAME_HEADER_t
{
	VBM_FRAME_HEADER_t()
	{
		memset(this,0,sizeof(VBM_FRAME_HEADER_t));
	}
...
typedef struct VBM_RENDER_CHUNK_t
{
	VBM_RENDER_CHUNK_t()
	{
		memset(this,0,sizeof(VBM_RENDER_CHUNK_t));
	}
...
typedef struct VBM_MATERIAL_t
{
	VBM_MATERIAL_t()
	{
		memset(this,0,sizeof(VBM_MATERIAL_t));
	}
